# Hub JWT 권한 시스템 통합 가이드 (ExamHub)

## 개요

Hub(중앙 인증 서버)에서 발급한 JWT 토큰에 포함된 앱별 권한 정보를 확인하여, ExamHub 앱의 특정 기능에 대한 접근을 제어합니다.
...
```json
{
  ...
  "permissions": {
    "examhub": {
      ...
      "features": ["mock-exam", "analysis", "prediction", "statistics"]
    }
  }
}
```
...
export class MockExamController {
  ...
  // 무료 사용자도 접근 가능 (기본 점수 조회)
  @Get('basic-score')
  getBasicScore() {
    return { message: '기본 점수 조회 - 무료' };
  }

  // 'mock-exam' 기능 권한이 필요한 API
  @Get('detailed')
  @UseGuards(HubPermissionGuard)
  @RequireFeature('mock-exam')
  getDetailedExam() {
    return { message: '상세 모의고사 분석 - basic 이상' };
  }
  ...
  getAnalysis() {
    return { message: '심화 분석 - premium 플랜 필요' };
  }

  // 'statistics' 기능 권한이 필요한 API
  @Get('statistics')
  @UseGuards(HubPermissionGuard)
  @RequireFeature('statistics')
  getStatistics() {
    return { message: '통계 분석 - premium 플랜 필요' };
  }
}
```
...
```json
{
  "plan": "basic",
  "expires": "2025-12-31T23:59:59Z",
  "features": ["mock-exam", "basic-analysis"]
}
```
- 모의고사 상세 조회 + 기본 분석 가능
- 만료일 존재
...
```
- 모든 기능 사용 가능 (모의고사, 분석, 예측, 통계, 내보내기)
- 만료일 존재
...
```json
{
  "success": false,
  "statusCode": 403,
  "message": "'analysis' 기능을 사용할 권한이 없습니다."
}
```

## Hub와 JWT 시크릿 키 공유

ExamHub와 Hub는 **동일한 JWT 시크릿 키**를 사용해야 합니다.
...
```bash
curl -X GET http://localhost:4003/mock-exam/analysis \
